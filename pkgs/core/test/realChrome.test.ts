 ...
 * these tests launch an actual Chrome via `launchChromeNode` and exercise the
 * public Handstage API end-to-end over a real CDP pipe: navigation,
 ...
 *
 * The whole suite is defined twice — once with `headless: true` and once with
 * `headless: false` — so the exact same behaviour is verified against both a
 * headless browser and a headful (windowed) browser. Headful Chrome needs a
 * display; the headful pass self-skips when no `DISPLAY` is available so the
 * file stays green in headless-only environments.
 *
 ...
 * per {@link LAUNCH_MODES} entry so the same assertions run against both a
 * headless and a headful browser.
 */
function defineRealChromeSuite(mode: LaunchMode): void {
	const realChrome = describe.skipIf(!mode.enabled)

	async function launchHandstage(): Promise<V3> {
		const chrome = await launchChromeNode({ headless: mode.headless })
		return connectLocal(chrome)
	}

	realChrome(`real Chrome [${mode.label}] — page basics`, () => {
		let v3: V3
	...
	realChrome(`real Chrome [${mode.label}] — pages & contexts`, () => {
		let v3: V3
	...
	realChrome(`real Chrome [${mode.label}] — connection lifecycle`, () => {
		test(
...
// Run the entire suite once per launch mode (headless, then headful).
for (const mode of LAUNCH_MODES) {
	defineRealChromeSuite(mode)
}
